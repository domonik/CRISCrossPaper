...
base_params = {
    ...
    "lr": 1e-4,
    "patience": 10,
    "experiment": "CrossCellTypeTestNegativeSampling",
    "regression": False,
    "merge": "early",
...
for i in [10, 20, 30, 40, 50]:
    dataset = f"../datasets/final_df_IR40_{i}.csv"
    t_cell = pd.read_csv(dataset, sep=",")
    t_cell = t_cell.rename(
        {
    ...
    ds = f"datasets/TCellNegativeSamplingWithEpidir_{i}.tsv"
    t_cell.to_csv(ds, sep="\t", index=False)
    datasets[f"T_Cell_{i}"] = ds
    
dataset = "../datasets/Tcell_guideseq_17sgRNA_hard_negatives.csv"   
...
t_cell["epiDir"] = "AGTensorsCL:0000624"
ds = "datasets/TCellNegativeSamplingHard.tsv"
t_cell.to_csv(ds, sep="\t", index=False)
datasets[f"T_Cell_Hard"] = ds
...
if __name__ == "__main__":
    ...
    # Write to JSON
    with open("configs/fineTuningCRISPRATNegativeSampling.json", "w") as f:
        json.dump(fine_tune_configs, f, indent=2)

    with open("configs/TestCRISPRATNegativeSampling.json", "w") as f:
        json.dump(test_configs, f, indent=2)

    print(
        f"Wrote {len(fine_tune_configs)} configs to fineTuningCRISPRATNegativeSampling.json"
    )
    print(
        f"Wrote {len(test_configs)} configs to TestCRISPRATNegativeSampling.json"
    )
