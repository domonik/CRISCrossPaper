...
read_energy_parameters("src/energy_dics.pkl")
...
def calcRNADNAenergy(guideSeq, otSeq, GU_allowed=False):
    ...
    while i<epos:
        ...
        if loop_size<3:
            ...
            eng_con = float(RNA_DNA_internal_loop[loop_size])
            eng_con += float(RNA_DNA[0][guideSeq[i:i+2]][seq[i:i+2]]) 
            eng_con += float(RNA_DNA[0][guideSeq[j-1:j+1]][seq[j-1:j+1]])
